...
export class JobApplicationStore {
  ...
  private readonly _filterCompanyId = signal<string>('all');
  ...
  readonly filteredApplications = computed(() => {
    ...
    return apps.filter((app) => {
      ...
      // 3. Search Filter
      if (search) {
        const matches = [app.position, app.companyName, app.description, app.jobUrl]
          .filter(Boolean)
  ...
  selectApplication(id: string | null) {
    ...
    this.applicationService.getApplicationById(id).subscribe({
      next: (app) => {
        ...
        this._currentJob.set(app);
        this._tailoredResume.set(app.tailoredResume ?? null);
      },
  ...
  refineJobBrief(id: string, description: string) {
    ...
    return this.applicationService.refineJobBrief(id, description).pipe(
      tap(() => {
        this.notificationService.success('Job brief refined.', 'Vadis Guidance');
      }),
      catchError((err) => {
  ...
  analyzeApplication(id: string) {
    ...
    this.applicationService.analyzeJob(id).subscribe({
      next: (updatedApp) => {
        ...
        this._applications.update((apps) => apps.map((app) => (app.id === id ? updatedApp : app)));
        this._currentJob.set(updatedApp);
        ...
        if (updatedApp.matchScore > 0) {
          ...
          this.notificationService.info(
            'Analysis complete. Check Vadis Guidance for details.',
            'Vadis Analysis',
          );
        }
      },
      error: (err) => {
        ...
        const message =
          typeof err?.error === 'string'
            ? err.error
            : (err?.error?.message ?? 'Failed to analyze application. Please try again.');
        this.notificationService.error(message, 'Error');
      },
    });
  }
  ...
  generateAssets(id: string) {
    ...
    this.applicationService.generateAssets(id).subscribe({
      next: (res) => {
        this._applications.update((apps) =>
          apps.map((app) =>
            app.id === id
        ...
        const updated = this._applications().find((app) => app.id === id) || null;
        this._currentJob.set(updated);
        this._tailoredResume.set(res.tailoredResume);
        this._isGeneratingAsset.set(false);
        this._isProcessing.set(false);
        this.notificationService.success('Documents generated.', 'Vadis Guidance');
      },
      error: (err) => {
        console.error('Failed to generate assets', err);
        this._isGeneratingAsset.set(false);
        this._isProcessing.set(false);
        this._error.set('AI asset generation failed');
        const message =
          typeof err?.error === 'string'
            ? err.error
            : (err?.error?.message ?? 'Failed to generate assets');
        this.notificationService.error(message, 'Error');
      },
    });
  }
  ...
  generateResumeDraft(id: string) {
    ...
    return this.applicationService.optimizeResume(id).pipe(
      ...
      tap((content) => {
        ...
        this.notificationService.success('Resume draft generated.', 'Vadis Guidance');
      }),
      catchError((err) => {
  ...
  generateCoverLetterDraft(id: string) {
    ...
    return this.applicationService.generateCoverLetter(id).pipe(
      ...
      tap((content) => {
        ...
        this.notificationService.success('Cover letter draft generated.', 'Vadis Guidance');
      }),
      catchError((err) => {
