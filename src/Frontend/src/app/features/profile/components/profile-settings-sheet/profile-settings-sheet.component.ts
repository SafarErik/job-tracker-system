...
@Component({
  ...
  template: `
    <hlm-sheet
      ...
      [state]="uiService.isProfileSettingsOpen() ? 'open' : 'closed'"
      (closed)="uiService.closeProfileSettings()"
    >
      <hlm-sheet-content
      ...
      >
        <!-- Header -->
        <div
          class="px-8 py-6 space-y-1 border-b border-border/50 bg-muted/20 relative overflow-hidden"
        >
          <div
          ...
          <div class="flex items-center justify-between relative z-10">
            <div class="flex items-center gap-3">
              <div
                class="h-10 w-10 rounded-xl bg-background border border-border flex items-center justify-center text-foreground shadow-sm"
              >
            ...
            @for (item of navItems; track item.id) {
              ...
              >
                ...
                <div class="flex flex-col items-start">
                  ...
                  <span
                    class="text-[9px] opacity-70 leading-none"
                    [class.text-background/80]="activeSection() === item.id"
                    >{{ item.sub }}</span
                  >
                </div>
            ...
            <form [formGroup]="form" class="p-8 space-y-10">
              ...
              @if (activeSection() === 'intelligence') {
                <div class="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div class="space-y-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                      @for (tone of aiTones; track tone) {
                        <button
                          (click)="form.patchValue({ aiTone: tone })"
                  ...
                  <div class="space-y-6">
                    <div class="space-y-2">
                      ...
                      <div class="relative">
                        <ng-icon
                          name="lucideKey"
                          class="absolute left-3 top-3.5 text-muted-foreground h-4 w-4"
                        ></ng-icon>
                        <input
                          hlmInput
                          type="password"
                  ...
                  <!-- Deep Search Toggle -->
                  <div
                    class="p-6 rounded-2xl bg-muted/20 border border-border/50 flex items-center justify-between gap-6"
                  >
                    <div class="space-y-1">
                      ...
                      <p class="text-xs text-muted-foreground leading-relaxed">
                        Enable multithreaded web crawling via Firecrawl for deeper company
                        intelligence. High latency, high accuracy.
                      </p>
                    </div>
              ...
              @if (activeSection() === 'connectivity') {
                <div class="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <!-- Google Calendar -->
                  <div
                    class="p-6 rounded-2xl border border-border flex items-center justify-between"
                  >
                    ...
                    class="h-10 w-10 flex items-center justify-center rounded-xl bg-accent/10 text-accent"
                      ...
                      <div>
                        <div class="flex items-center gap-2">
                          ...
                          <span
                            ...
                            [class.text-muted-foreground]="
                              !form.get('googleCalendarConnected')?.value
                            "
                          >
                            {{
                        ...
                        <p class="text-xs text-muted-foreground mt-0.5">
                          Automated interview scheduling and time tracking.
                        </p>
                      </div>
                    </div>
                    <button
                  ...
                  <div class="space-y-4">
                    <label
                      class="text-[10px] font-black uppercase tracking-widest text-muted-foreground"
                      >Primary Intelligence Feed</label
                    >
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                      @for (board of ['LinkedIn', 'Indeed', 'Glassdoor']; track board) {
                        <div
                          class="flex items-center justify-between p-4 rounded-xl border border-border bg-muted/10"
                        >
                          <label [for]="board" class="text-xs font-bold">{{ board }}</label>
                          <hlm-switch
                            [id]="board"
                            [checked]="isJobBoardChecked(board)"
                            (changed)="toggleJobBoard(board)"
                          ></hlm-switch>
                        </div>
                      }
                    </div>
                  ...
                  <div class="grid grid-cols-2 gap-6">
                    <div class="space-y-2">
                      <label
                        hlmLabel
                        class="text-[10px] font-black uppercase tracking-widest text-muted-foreground"
                        >Baseline Currency</label
                      >
                      <brn-select formControlName="currency" placeholder="Select Currency">
                        <hlm-select-trigger
                          class="w-full rounded-xl h-11 border-border bg-muted/30"
                        >
                  ...
                  <div class="space-y-2">
                    ...
                    <div class="flex gap-2">
                      <button
                        hlmBtn
                        variant="outline"
                        class="flex-1 rounded-xl h-11 gap-2"
              ...
              @if (activeSection() === 'security') {
                <div class="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div class="space-y-4">
                    ...
                    <div class="space-y-4">
                      <div class="space-y-2">
                        <label hlmLabel class="text-xs font-bold text-muted-foreground"
                          >Identity Email (Immutable)</label
                        >
                        <div class="relative">
                          <ng-icon
                            name="lucideMail"
                  ...
                  <div class="space-y-4">
                    <button
                      hlmBtn
                      variant="outline"
                      class="w-full h-14 rounded-2xl justify-between px-6 border-border hover:bg-muted group"
                      (click)="exportDossier()"
                    >
                      <div class="flex items-center gap-4">
                        <div
                          class="h-8 w-8 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-500"
                        >
                    ...
                    <div
                      class="p-6 rounded-2xl border border-destructive/20 bg-destructive/5 space-y-4"
                    >
                      <div class="flex items-start gap-4">
                        <div
                          class="h-8 w-8 rounded-lg bg-destructive/10 flex items-center justify-center text-destructive shrink-0"
                        >
        ...
        <div class="p-6 border-t border-border bg-background/50 backdrop-blur-sm flex gap-3">
          <button
            ...
            variant="outline"
            size="lg"
            (click)="uiService.closeProfileSettings()"
...
export class ProfileSettingsSheetComponent {
  ...
  // Navigation State
  activeSection = signal<'intelligence' | 'connectivity' | 'preferences' | 'security'>(
    'intelligence',
  );

  navItems: {
    id: 'intelligence' | 'connectivity' | 'preferences' | 'security';
    label: string;
    sub: string;
    icon: string;
  }[] = [
  ...
  themeOptions: { id: Theme; label: string; icon: string; previewClass: string }[] = [
    {
      ...
      label: 'Paper',
      icon: 'lucideGlobe',
      previewClass: 'bg-zinc-100 text-zinc-900 border border-zinc-200',
    },
    {
      id: 'dark',
      label: 'Obsidian',
      icon: 'lucideGlobe',
      previewClass: 'bg-zinc-950 text-zinc-100 border border-zinc-800',
    },
    {
      id: 'system',
      label: 'System',
      icon: 'lucideLaptop',
      previewClass: 'bg-linear-to-br from-zinc-100 to-zinc-950 border border-zinc-400',
    },
  ];
  ...
  form = this.fb.group({
    ...
    salaryPeriod: ['Yearly'],
    language: ['English'],
    jobBoards: this.fb.control<string[]>([]),
  });
